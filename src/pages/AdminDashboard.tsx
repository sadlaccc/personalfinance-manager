...
export default function AdminDashboard() {
  ...
  const StatCard = ({
    ...
    label: string;
    value: number | string;
    tint?: 'primary' | 'accent' | 'income' | 'warning' | 'info';
    trend?: string;
  }) => {
    const tintClasses: Record<string, { icon: string; ring: string; text: string }> = {
      primary: { icon: 'bg-primary/10 text-primary', ring: 'ring-primary/10', text: 'text-primary' },
      accent: { icon: 'bg-accent/15 text-accent-foreground', ring: 'ring-accent/15', text: 'text-accent-foreground' },
      income: { icon: 'bg-income/10 text-income', ring: 'ring-income/10', text: 'text-income' },
      warning: { icon: 'bg-warning/10 text-warning', ring: 'ring-warning/10', text: 'text-warning' },
      info: { icon: 'bg-info/10 text-info', ring: 'ring-info/10', text: 'text-info' },
    };
    const t = tintClasses[tint];
    return (
      <motion.div whileHover={{ y: -2 }} transition={{ type: 'spring', stiffness: 300, damping: 22 }}>
        <Card className={`border-border/60 ring-1 ${t.ring} overflow-hidden relative`}>
          <CardContent className="p-3.5 flex items-center gap-3">
            <div className={`p-2.5 rounded-xl ${t.icon} shrink-0`}>
              <Icon className="h-4 w-4" />
            </div>
            <div className="min-w-0 flex-1">
              <p className="text-[11px] text-muted-foreground truncate leading-tight uppercase tracking-wide">
                {label}
              </p>
              <p className="text-xl font-bold font-display tracking-tight leading-tight mt-0.5">
                {value}
              </p>
            </div>
            {trend && (
              <span className={`text-[10px] font-semibold ${t.text} shrink-0`}>{trend}</span>
            )}
          </CardContent>
        </Card>
      </motion.div>
      ...
      <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-3">
        ...
        <motion.div variants={itemVariants} className="grid gap-3 grid-cols-2 lg:grid-cols-6">
          {usersLoading || subsLoading ? (
            ...
            <>
              <StatCard icon={Users} label="Total Users" value={totalUsers} tint="primary" />
              <StatCard icon={TrendingUp} label="New (7d)" value={`+${newThisWeek}`} tint="income" trend="7d" />
              <StatCard icon={Activity} label="New (30d)" value={`+${newThisMonth}`} tint="info" trend="30d" />
              <StatCard icon={Clock} label="Active (7d)" value={activeRecently} tint="primary" />
              <StatCard icon={Crown} label="Paid Users" value={paidUsers} tint="accent" />
              <StatCard icon={AlertTriangle} label="On Trial" value={trialUsers} tint="warning" />
            </>
          )}
        </motion.div>
