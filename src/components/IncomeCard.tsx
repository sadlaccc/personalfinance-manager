...
import { Pencil, Trash2, TrendingUp, Calendar, ArrowDownToLine } from 'lucide-react';
...
export function IncomeCard({ income, onEdit, onDelete }: IncomeCardProps) {
  ...
  const isCarryover = income.isCarryover;

  return (
    <div className={cn(
      "group relative bg-card border rounded-2xl p-5 transition-all duration-300 animate-fade-in",
      isCarryover
      ...
      <div className={cn(
        ...
        isCarryover ? "bg-primary" : categoryColors[income.category]
      )} />
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-2 flex-wrap">
            {isCarryover ? (
              <span className="text-xs font-medium px-2 py-1 rounded-full bg-primary/10 text-primary inline-flex items-center gap-1">
          ...
          {!isCarryover && income.frequency !== 'monthly' && income.frequency !== 'one-time' && (
