...
export function ExportReportDialog({
  ...
  const generateCSV = () => {
    const lines: string[] = [];
    lines.push(`Pesa Safi Monthly Report - ${monthLabel}`);
    lines.push('');
    lines.push('Summary');
  ...
  const generatePDFContent = () => {
    const html = `
      ...
      <html>
      <head>
        <title>Pesa Safi Report - ${monthLabel}</title>
        <style>
      ...
      <body>
        <h1>📊 Pesa Safi Monthly Report</h1>
        ...
        <div class="footer">
          <p>Generated by Pesa Safi • ${format(new Date(), 'PPP')}</p>
        </div>
      </body>
      </html>
    `;
  ...
  const handleExport = async () => {
    ...
    try {
      if (format_ === 'csv') {
        ...
        link.download = `pesa-safi-report-${format(selectedMonth, 'yyyy-MM')}.csv`;
        link.click();
